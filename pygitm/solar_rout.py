...
def subsolar_point(ut):
    ...
    doy   = stime.get_julian_solar_day(ut)
    sslat = solar.get_declination(doy)
...
def get_solar_terminator_xyz_matrix(ut, alt=0.0):
    ...
    jd  = stime.get_julian_solar_day(ut)
    jde = stime.get_julian_ephemeris_day(ut)
    jce = stime.get_julian_ephemeris_century(jde)
    #jme = stime.get_julian_ephemeris_millenium(jce)
...
def get_terminator_lat_lon_coordinates(s_vect, z_vect, num_latlon=10):
    ...
    for z_scale in zinc:
        ...
        for i,s in enumerate(s_vect):
	        xyz = [z_scale * z_vect[i] - s_scale * s]
        ...
        for i,s in enumerate(s_vect):
	        xyz = [s_scale * s + z_scale * z_vect[i]]
        (gdlon, gdlat, gdalt) = spherical_xyz2lat_lon(xyz[0], xyz[1], xyz[2])
