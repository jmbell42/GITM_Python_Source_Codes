#!/usr/bin/env python
#-----------------------------------------------------------------------------
# $Id: gitm_plot_rout.py,v 1.19 2014/03/27 11:27:35 agburr Exp $
# gitm_plot_rout
#
...
#    if string.lowercase(units) == "degrees":
...
try:
    ...
    def find_sunside_twilight_sza(ut, lat_top, lat_bot, lon_top, lon_bot,
        ...
        for sec in np.arange(0.0, 86400.0, 1800.0):
            ...
            sza_top = np.array([90.0 - solar.get_altitude(lat_top[i], l, t)
                                if glon_to_localtime(t, l) < 12.0
                                else -90.0 + solar.get_altitude(lat_top[i], l, t)
                                for i,l in enumerate(lon_top)])
            sza_bot = np.array([90.0 - solar.get_altitude(lat_bot[i], l, t)
                                if glon_to_localtime(t, l) < 12.0
                                else -90.0 + solar.get_altitude(lat_bot[i], l, t)
