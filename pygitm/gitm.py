...
class GitmBin(PbData):
    ...
    def _read(self, varlist, newfile=True):
        ...
        Read binary file.
        '''
   #     print('Process for Reading binary file')
        from re import sub
        ...
        if (recLen>10000) or (recLen<0):
            #print('recLen >10000:', recLen>10000)
            #print('recLen <0    :', recLen<0)
...
#        print('Get Version ', self.attrs['version'])
        (oldLen, recLen)=unpack(endChar+'2l',f.read(8))
 #       print('Reading Header and Footer:', (oldLen,recLen))

        # Read grid size information.
        (self.attrs['nLon'],self.attrs['nLat'],self.attrs['nAlt'])=\
            unpack(endChar+'lll',f.read(recLen))

 #       print('Get Grid (Lons,Lats,Alts) ', self.attrs['nLon'], \
#                self.attrs['nLat'], self.attrs['nAlt'])
 ...
 #       print('Reading Header and Footer:', (oldLen,recLen))
        ...
        for i in range(self.attrs['nVars']):
            ...
            var[i] = var[i].decode()   # change from byte to string type
            (oldLen, recLen)=unpack(endChar+'2l',f.read(8))
      #  print(var)
      ...
      #      print("Var(i) =",i,var[i])
        ...
        # Need timezone for pysolar to work properly
        # Get pytz information and use timezone('UTC') to set this
        utc = pytz.timezone('UTC')
        self['time']=dt.datetime(yy,mm,dd,hh,mn,ss,ms//1000,tzinfo=utc)
    ...
    def calc_magdi(self):
        ...
        if mag is not None:
            ...
            if not 'Magnetic Latitude' in self.keys():
    ...
    def append_units(self):
        ...
        unit_dict = {"Altitude":"m", 
                     ...
                     "N(!U4!NS)":"kg \, m^{-3}", "N2 Mixing Ratio":"",
                     "NO":"kg \, m^{-3}", "NO!U+!N":"kg \, m^{-3}",
                     "O!D2!N":"kg \, m^{-3}", "O(!U1!ND)":"kg \, m^{-3}",
                     "O!D2!U+!N":"kg \, m^{-3}", "O(!U2!ND)!":"m^{-3}",
                     "O(!U2!ND)!U+!N":"m^{-3}", "O(!U2!NP)!U+!N":"kg \, m^{-3}",
                     "O(!U2!NP)!U+!N":"kg \, m^{-3}",
                     "O(!U3!NP)":"kg \, m^{-3}", "O_4SP_!U+!N":"kg \, m^{-3}",
                     ...
                     "CO!D2!N":"kg \, m^{-3}",  "DivJu FL":"", "DivJuAlt":"",
